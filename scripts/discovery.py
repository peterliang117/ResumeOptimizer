...
try:
    from job_store import DEFAULT_DB, database_enabled, export_legacy_csv, initialize, score_adjustment, upsert_job
    ...
    from scripts.job_store import DEFAULT_DB, database_enabled, export_legacy_csv, initialize, score_adjustment, upsert_job
...
def main() -> int:
    ...
    if args.queue:
        if not result["eligible"]:
            raise SystemExit("Candidate failed hard gates; it was not queued.")
        initialize(args.db)
        ...
        values.update(
            {
                ...
                "expires_at": (
                    posted_at + timedelta(hours=read_search_criteria(args.criteria).maximum_freshness_hours)
        ...
        upsert_job(values, path=args.db)
